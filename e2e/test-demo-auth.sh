...
chmod +x "$stub_bin/npm" "$stub_bin/git" "$stub_bin/gh" "$stub_bin/security" "$runtime_root/bin/outfitter"
...
run_live_mcp_failure_case() {
  ...
  if output=$(cd "$repo_root" && HOME="$case_root/host-home" \
    ...
    echo "FAIL  outfitter mode accepted a missing GitHub MCP server" >&2
    exit 1
  fi
  case "$output" in
  ...
  echo "PASS  outfitter mode rejects a missing GitHub MCP server before reset"
}
