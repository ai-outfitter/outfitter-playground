...
preflight_pi_auth() {
  ...
  # Provider authentication does not need repository credentials. Keep the
  # GitHub token out of this model-only probe even though live-mode GitHub
  # access has already been validated.
  auth_probe_github_token=$demo_github_token
  demo_github_token=""
  ...
  demo_github_token=$auth_probe_github_token
  unset auth_probe_github_token
  cat "$auth_probe_log"
...
EOF
exec env -u GH_TOKEN -u GITHUB_TOKEN -u GITHUB_PERSONAL_ACCESS_TOKEN \
  HOME="$demo_home" \
  XDG_CONFIG_HOME="$demo_home/.config" \
  PATH="$demo_path" \
  NPM_CONFIG_AUDIT=false \
